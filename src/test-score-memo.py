...
if __name__ == "__main__":
    ...
    if model_name == "ResNet-50":
        if wb:
            wandb.init(project=project_name, name=("SCOREMEMO_" + norm_adaptation_tag + conf_tresh_tag + "_ResNet50"), config=hyperparams)
        test_resnet50(hyperparams, A, norm_adaptation, DEVICE, wb)
    elif model_name == "ViT-B/16":
        if wb:
            wandb.init(project=project_name, name=("SCOREMEMO_" + conf_tresh_tag + "_ViT-B/16"), config=hyperparams)
